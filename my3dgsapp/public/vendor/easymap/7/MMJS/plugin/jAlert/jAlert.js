...
;(function($) {
	...
	$.fn.jAlert = function(options) {
		...
		var themes = ['default', 'green', 'dark_green', 'red', 'dark_red', 'black', 'brown', 'gray', 'dark_gray', 'blue', 'dark_blue', 'yellow'],
            ...
            classes = ['ja_animated'], //array of classes that get joined together with a space between on the jalert div
		...
		if( !options.id )
		{
			var unique = Date.now().toString() + Math.floor(my_fix_random() * 100000);
		...
		else if( alert.ajax )
		{
			...
			alert.onOpen = [function(elem){
				$.ajax(elem.jAlert().ajax, {
					...
					{
                        elem.find('.ja_media_wrap').replaceWith(jqXHR[decodeURIComponent(encodeURIComponent('responseText'))]);
		...
		var getBtnHTML = function(btn){
			...
			if(typeof btn.id == 'undefined'){ var unique = Date.now().toString() + Math.floor(my_fix_random() * 100000); btn.id = 'ja_btn_' + unique; }
	...
	$.fn.jAlert.defaults = {
		...
		'showAnimation': 'MMfadeInUp',
		'hideAnimation': 'MMfadeOutDown',
