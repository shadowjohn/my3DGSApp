...
var Windy = function (params) {
	...
	const PARTICLE_LINE_WIDTH = params.lineWidth || 2;                           // line width of a drawn particle
	const PARTICLE_MULTIPLIER = params.particleMultiplier || 1 / 600;            // particle count scalar (completely arbitrary--this values looks nice)
	const PARTICLE_REDUCTION = (Math.pow(window.devicePixelRatio, 1 / 3) || 1.6);   // multiply particle count for mobiles by this amount
	var FRAME_RATE = params.frameRate || 15, FRAME_TIME = 1000 / FRAME_RATE;   // desired frames per second
	...
	var createField = function (columns, bounds, callback) {
		...
		field.randomize = function (o) {  // UNDONE: this method is terrible
			...
			var safetyNet = 0;
			do {
				x = Math.round(Math.floor(my_fix_random() * bounds.width) + bounds.x);
				y = Math.round(Math.floor(my_fix_random() * bounds.height) + bounds.y)
	...
	var animate = function (bounds, field) {
		...
		for (var i = 0; i < particleCount; i++) {
			particles.push(field.randomize({ age: Math.floor(my_fix_random() * MAX_PARTICLE_AGE) + 0 }));
		}
	...
	var setWindSpeed = function (floatData) {
		var _t = floatData || 15;
		FRAME_TIME = 1000 / _t;
	}
